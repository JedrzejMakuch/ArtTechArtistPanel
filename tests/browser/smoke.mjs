...
try {
    ...
    const profileCode = (await page.locator('.profile-details .hint').filter({ hasText: 'Profile code:' }).textContent()).split(':')[1].trim();
    const profileShare = url(`view/profile/${profileCode}`);
    assert.equal(await page.locator('.profile-details .share-link a').getAttribute('href'), profileShare);
    await page.locator('.profile-details').getByRole('button', { name: 'Copy link', exact: true }).click();
    assert.equal(await page.evaluate(() => navigator.clipboard.readText()), profileShare);
    await page.locator('.profile-details').getByRole('button', { name: 'Show QR', exact: true }).click();
    assert.equal(await page.locator('.profile-details .share-qr svg').count(), 1);
    assert.ok((await page.locator('.profile-details a[download]').getAttribute('href')).startsWith('data:image/svg+xml;base64,'));
